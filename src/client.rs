...
fn main() {
    let mut con = Connection::new_client_connection(255);
    ...
    chat.push((String::from("Connected."), false));
    loop {
        let msg = con.receive_message();
        if handle_server_message(&con, &mut chat, msg, sent_time) {
            break;
        }
